...
| --- | --- | --- | --- |
| L1 | ~~**Payer keys on disk**, not in an enclave. The core custody claim is unbacked in the running system.~~ | ~~**High**~~ | **Fixed 2026-08-23.** Deployed to ROFL: the signer reports `store=ROFL enclave via /run/rofl-appd.sock`, and goal #52's payer appears in the enclave log, in the vault, and **not** in `.keys/payers.json`. The file store remains for laptop runs; `SIGNER_REQUIRE_ROFL=true` is the switch that forbids it, and is **not yet set** — that is the last step of A1. |
