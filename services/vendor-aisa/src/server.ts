...
export function createVendorApi(options: HandlerOptions): Server {
  return createServer((req, res) => {
    void (async () => {
      ...
      // Query string included: the handler needs `q` and `tier`, and builds the
      // advertised `resource` from the same request line so the 402 and the
      // paid retry cannot disagree about what was bought.
 ...
 * service holds a key that spends money on an external account, so reaching it
